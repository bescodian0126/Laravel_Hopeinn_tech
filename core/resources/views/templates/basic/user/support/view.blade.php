...
@section('content')
    <div class="{{ $layout == 'frontend' ? 'padding-top padding-bottom container' : 'dashboard-inner' }}">
        <div class="row">
            <div class="col-md-12">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('ticket.reply', $myTicket->id) }}"
                            ...
                            <div class="text-end mb-sm-0 mb-1">
                                <button type="button" class="btn btn--base btn--sm addFile w-auto h-auto"><i class="fa fa-plus"></i>
                                    @lang('Add New')</button>
                            </div>
                            <div class="form-group">
                                <label class="form-label mb-0">@lang('Attachments')</label> <small
                                    class="text--danger">@lang('Max 5 files can be uploaded'). @lang('Maximum upload size is')
                                    {{ ini_get('upload_max_filesize') }}</small>
                                <input type="file" name="attachments[]" class="form-control form--control h-auto" />
