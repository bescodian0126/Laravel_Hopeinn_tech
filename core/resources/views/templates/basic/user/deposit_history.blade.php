    ...
    <div class="dashboard-inner">
        <div class="mb-4">
            <div class="d-flex justify-content-between">
                ...
                <span>
                    <a href="{{ route('user.deposit.index') }}" class="btn btn--base btn--smd">
                        @lang('Deposit Now')
                    </a>
                </span>
            </div>
                    ...
                    <h2 class="accordion-header" id="h-{{ $loop->iteration }}">
                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                            ...
                            <div class="col-lg-4 col-sm-3 col-4 order-sm-3 order-2 text-end amount-wrapper">
                                <p><b>{{ showAmount($deposit->amount) }} {{ __($general->cur_text) }}</b></p>
                        ...
                        <div class="accordion-body">
                            <ul class="caption-list">
                                ...
                                <li>
                                    ...
                                    <span class="value">{{ showAmount($deposit->amount + $deposit->charge) }}
                                        {{ __($general->cur_text) }} x {{ showAmount($deposit->rate) }}
                                ...
                                <li>
                                    ...
                                    <span class="value">
                                        @php echo $deposit->statusBadge @endphp @if ($deposit->method_code >= 1000)
                                            <button type="button" class="btn p-0 detailBtn" data-info="{{ $details }}"
                                                @if ($deposit->status == Status::PAYMENT_REJECT) data-admin_feedback="{{ $deposit->admin_feedback }}" @endif><i
